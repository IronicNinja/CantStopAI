    ...
    while True:
        ...
        if RIG_TYPE.lower() == "random":
            ...
            if RIG_TYPE.lower() == "sqrt":
                RIG = (1-RIG)/math.sqrt(curr_success+1)
            elif RIG_TYPE.lower() == "exp":
                RIG = pow((1-RIG), curr_success)
            elif RIG_TYPE.lower() == "constant":
                RIG = 1-RIG

        #global_arr.append(RIG)
...
def play(ITERATIONS, th=0.9, PRINT_STATS=False, RIG_TYPE="EXP"):
    ...
    while True:
        ...
        for i in range(len(possible_moves)):
            ...
            for move in possible_moves[i]:
                ...
                if PLAYER_MOVE_now:
                    hypo_A_now[move] += 1
                    if hypo_A_now[move] >= board_end[move]:
                        hypo_completed_now.add(move)
                        hypo_A_score_now += 1
                else:
                    hypo_B_now[move] += 1
                    if hypo_B_now[move] >= board_end[move]:
                        hypo_completed_now.add(move)
                        hypo_B_score_now += 1
        ...
        if win_percentage_cont[0]/win_percentage_cont[1] <= th or len(chips_list_org) < 3:
...
play(1600, th=0.95, PRINT_STATS=True, RIG_TYPE="exp")

#sns.distplot(global_arr)
#plt.show()